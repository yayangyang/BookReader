package com.imooc.brvaheasyrecycleview.view.readview;
...
public class PageFactory {
    ...
    public synchronized void onDraw(Canvas canvas) {
        ...
        if (mLines.size() > 0) {
            ...
            Log.e("marginHeight","ww"+marginHeight);
            Log.e("mLineSpace","ww"+mLineSpace);
            Log.e("mNumFontSize","ww"+mNumFontSize);
            Log.e("mFontSize","ww"+mFontSize);
            int a=mLineSpace<<1;
            Log.e("mLineSpace*2","ww"+a);
            // 绘制标题
            canvas.drawText(chaptersList.get(currentChapter - 1).title, marginWidth, y, mTitlePaint);
//            canvas.drawText(chaptersList.get(currentChapter - 1).title, marginWidth+100, 84, mTitlePaint);
    ...
    private Vector<String> pageDown() {
        ...
        while ((lines.size() < mPageLineCount) && (curEndPos < mbBufferLen)) {
            ...
            Log.e("strParagraph","ww"+strParagraph);
            strParagraph = strParagraph.replaceAll("\r\n", "  ")
                    .replaceAll("\n", " "); // 段落中的换行符去掉，绘制的时候再换行

            while (strParagraph.length() > 0) {
                //根据给出最大宽度计算一行能最大显示字数(自己添加)
                int paintSize = mPaint.breakText(strParagraph, true, mVisibleWidth, null);

                LogUtils.e("paintSize:"+paintSize);
                LogUtils.e(strParagraph.substring(0, paintSize));
